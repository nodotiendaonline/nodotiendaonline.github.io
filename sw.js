...
const CACHE_NAME = "nodo-shell-v2";
...
//
// IMPORTANTE: solo interceptamos pedidos hacia NUESTRO propio sitio.
// Si dejáramos pasar por acá pedidos a otros dominios (Google Analytics,
// Firebase, fuentes de Google, etc.), podemos alterar esas respuestas
// sin querer y romper cosas como el conteo de visitas.
self.addEventListener("fetch", (event) => {
  if (event.request.method !== "GET") return;

  const requestUrl = new URL(event.request.url);
  if (requestUrl.origin !== self.location.origin) return; // dejamos pasar de largo lo externo
